...
def main(config):
    ...
    # checkpoint = torch.load('output/giga2_patch16_stride4_window7_448_panda_x0.5/default/ckpt_epoch_293.pth', map_location='cpu')
    ...
    for img_path in tqdm.tqdm(condition_img_paths):
        ...
        ori_img = ori_img[112:-112, 112:-112, :]
        # img = img.resize((size[0] // 2, size[1] // 2))
        # img = img[112:-112 , 112:-112, :]
        ...
        if output > 0.5 :
            count += 1
            plt.imsave(os.path.join(visualization_root, name), attn.cpu().numpy(), cmap='Blues')
            cv2.imwrite(os.path.join(visualization_root, name.replace('.jpg', '.png')), ori_img[:, :, [2,1,0]])
    print(count, len(condition_img_paths))
        # np.save(os.path.join(visualization_root, name.replace('.jpg', '.npy')), output.cpu().numpy())
