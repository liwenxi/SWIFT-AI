...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix, cls_tokens):
        """ Forward function.
        Args:
            x: Input feature, tensor size (B, H*W, C).
            H, W: Spatial resolution of the input feature.
            mask_matrix: Attention mask for cyclic shift.
            cls_tokens: Additional token for classification.
        """
        B, L, C = x.shape
        H, W = self.H, self.W
        ...
        # cls_token
        x_windows = torch.cat((cls_tokens, x_windows), dim=1)

        # W-MSA/SW-MSA
        attn_windows, attn = self.attn(x_windows, mask=attn_mask)  # nW*B, window_size*window_size, C

        # cls_token
        cls_tokens = attn_windows[:, :1]
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Conv2d(dim, dim*2, kernel_size=4, stride=4)
        self.norm = norm_layer(4 * dim)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, _, _ = x.shape
        nW, _ = mask_windows.shape
        cls_tokens = self.cls_token.expand(B*nW, -1, -1)

        for blk in self.blocks:
            blk.H, blk.W = H, W
            if self.use_checkpoint:
                x, cls_tokens, attn = checkpoint.checkpoint(blk, x, attn_mask, cls_tokens)
            else:
                x, cls_tokens, attn = blk(x, attn_mask, cls_tokens)
        if self.downsample is not None:
            x_down = self.downsample(x, H, W)
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x, cls_tokens, attn, H, W, x_down, Wh, Ww
        else:
            return x, cls_tokens, attn, H, W, x, H, W


class PatchEmbed(nn.Module):
    """ Image to Patch Embedding
    Args:
        patch_size (int): Patc h token size. Default: 4.
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        stride_size = to_2tuple(patch_size*4)
