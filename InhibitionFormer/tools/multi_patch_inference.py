...
def inference_detector_by_patches(model,
    ...
    # windows = slide_window(width, height, [(2000*200, 1200*200)], [(2000*200-600, 1200*200-600)])
    #
    ...
    while True:
        ...
        print("a batch:", time.time()-temp_point)
        if end >= len(windows):
            break
        start += bs
    global all_time
...
def main(args):
    ...
    with open('cascade_giga.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    device = 'cpu'
    all_result = []
    outputs = mmcv.load('cascade_giga.pkl')
    ...
    for results in outputs:
        ...
        for tran_dets in results:
            ...
            ####
            nms_dets = nms_dets.cpu().numpy()
            ####
            # nms_dets = nmbs(nms_dets[:, :5], nms_dets[:, -1],
            #                           0.6)
            # nms_dets = nmbs(nms_dets[:, :5].contiguous(), nms_dets[:, -1].contiguous(),
            #                           0.7)
            # nms_dets = tran_dets
    ...
    # print(all_result)
    with open('cascade_giga.nms.pkl', 'wb') as f:
        pickle.dump(all_result, f)